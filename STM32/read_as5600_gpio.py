...
class encoder_as5600():
    def __init__(self, round_gate=0.5):
        self.uart = UART(2, baudrate=115200, bits=8, parity=None, stop=1)  # 设置串口号2和波特率
#         self.uart = UART(1, 115200)  # 设置串口号2和波特率
    ...
    def read_stm32_port(self):
        return (stm.mem32[stm.GPIOD + stm.GPIO_IDR] & 0xFFFF)  # 读取16个引脚状态
...
#         if data!='':
#             self.val = data / 4095
    def mask1357(self,data):
        cleared_data = data & ~0x00AA  # 0x00AA = 0000000010101010（16位掩码）
        # Step 2: 重新组合剩余位（位15-0，跳过第1、3、5、7位）
        result = (
            ((cleared_data & 0xFF80)  >> 4) |  # 位8  → 新位4
            ((cleared_data & (1 << 6))  >> 3) |  # 位6  → 新位3
            ((cleared_data & (1 << 4))  >> 2) |  # 位4  → 新位2
            ((cleared_data & (1 << 2))  >> 1) |  # 位2  → 新位1
            ((cleared_data & (1 << 0))  >> 0)    # 位0  → 新位0
        )
        return result

    def get_raw(self):  # 串口获取数据
        raw = self.read_stm32_port()
        data=self.mask1357(raw)#把1、3、5、7位置的数值置零
#         print(raw,data)
        self.val = data / 4095
